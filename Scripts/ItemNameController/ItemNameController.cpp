...
void ItemNameController::Update()
{
	for (int i = 0; i != 10; ++i) {
		if (itemList[i].show)
		{
			...
			math::float2 pos = itemList[i].actualPosition - math::float2((itemBackground[i]->GetComponent<Transform2D>()->getSize().x / 2) - 10.0f, 5.0f);
			...
			itemBackground[i]->GetComponent<Transform2D>()->SetSize(math::float2(myText->text.size() * 14, 30));
...
void ItemNameController::SetNameBar(unsigned uid, ItemRarity rarity)
{
	for (int i = 0; i != 10; i++) 
	{
		if (!itemList[i].show)
		{
			...
			itemBackground[i] = (uiName[i]->children).front();
			itemTypeName = (uiName[i]->children).back();
			myText = itemTypeName->GetComponent<Text>();
			itemList[i].rarity = rarity;
			myText->text = go->name;
			itemBackground[i]->GetComponent<Transform2D>()->SetPositionUsingAligment(itemList[i].actualPosition);
			...
			itemBackground[i]->SetActive(true);
