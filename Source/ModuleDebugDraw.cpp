...
bool ModuleDebugDraw::Init(JSON *json) {
...
void ModuleDebugDraw::Draw(const ComponentCamera &camera, unsigned fb_width, unsigned fb_height) {
    implementation->width     = fb_width;
    implementation->height    = fb_height;
    implementation->mvpMatrix = camera.frustum->ProjectionMatrix() *  camera.frustum->ViewMatrix();
	dd::flush();
}
